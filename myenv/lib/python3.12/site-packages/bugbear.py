...
class BugBearVisitor(ast.NodeVisitor):
        ...
        for key in duplicate_keys:
            ...
            for index in key_indices:
                ...
                if value in seen:
                    ...
                    self.errors.append(
                        B041(key_node.lineno, key_node.col_offset)
                    )
                seen.add(value)
    ...
    def check_for_b013_b014_b029_b030(
        ...
        if len(names) == 0 and not bad_handlers and not ignored_handlers:
            ...
            maybe_error = _check_redundant_excepthandlers(
                names, node, self.in_trystar
            )
    ...
    def check_for_b019(self, node) -> None:
        ...
        resolved_decorators = (
            ".".join(compose_call_path(decorator))
    ...
    def check_for_b023(self, loop_node) -> None:  # noqa: C901
        ...
        for node in ast.walk(loop_node):
            ...
            if isinstance(node, FUNCTION_NODES) and node not in safe_functions:
                ...
                for err in errors:
                    if (
                        err.vars[0] not in argnames
                        and err not in self._b023_seen
                    ):
    ...
    def check_for_b024_and_b027(self, node: ast.ClassDef) -> None:  # noqa: C901
        ...
        def is_abc_class(value, name: str = "ABC"):
            ...
            if isinstance(value, ast.keyword):
                return value.arg == "metaclass" and is_abc_class(
                    value.value, "ABCMeta"
                )
            # class foo(ABC)
            # class foo(abc.ABC)
        ...
        def is_abstract_decorator(expr):
            return (
                isinstance(expr, ast.Name) and expr.id[:8] == "abstract"
            ) or (
        ...
        def empty_body(body) -> bool:
            ...
            return all(
                ...
                or (
                    ...
                    and is_str_or_ellipsis(stmt.value)
                )
                for stmt in body
            )
    ...
    def check_for_b031(self, loop_node) -> None:  # noqa: C901
        ...
        if isinstance(loop_node.iter, ast.Call):
            node = loop_node.iter
            if (
                isinstance(node.func, ast.Name) and node.func.id in ("groupby",)
            ) or (
                isinstance(node.func, ast.Attribute)
    ...
    def check_for_b035(self, node: ast.DictComp) -> None:
        ...
        if isinstance(node.key, ast.Constant):
            self.errors.append(
                B035(
                    node.key.lineno, node.key.col_offset, vars=(node.key.value,)
                )
            )
        elif isinstance(node.key, ast.Name):
            if (
                node.key.id
                not in self._get_dict_comp_loop_and_named_expr_var_names(node)
            ):
                self.errors.append(
                    B035(
                        node.key.lineno,
    ...
    def check_for_b040_usage(self, node: ast.expr | None) -> None:
        ...
        for n in superwalk(node):
            if (
                ...
                and n.id == self.b040_caught_exception.name
            ):
                self.b040_caught_exception = None
                break
    ...
    def check_for_b902(  # noqa: C901 (too complex)
        ...
        if any(
            basetype in bases for basetype in ("type", "ABCMeta", "EnumMeta")
        ):
        ...
        for duplicate in duplicates:
            self.errors.append(
                B025(
                    node.lineno,
                    node.col_offset,
                    vars=(duplicate, self.in_trystar),
                )
            )
    ...
    def check_for_b906(self, node: ast.FunctionDef) -> None:
        ...
        for n in itertools.chain.from_iterable(
            ast.walk(nn) for nn in node.body
        ):
            ...
            if (
                ...
                if (
                    ...
                    if (
                        ...
                        or not isinstance(
                            value.format_spec.values[0], ast.Constant
                        )
                    ):
                        current_mark = variable = None
                        continue
                    ...
                    # char - strip it
                    if (
                        len(format_specifier) > 1
                        and format_specifier[1] in "<>=^"
                    ):
    ...
    def check_for_b034(self, node: ast.Call) -> None:
        ...
        def check(num_args: int, param_name: str) -> None:
            if len(node.args) > num_args:
                ...
                self.errors.append(
                    B034(
                        arg.lineno, arg.col_offset, vars=(func.attr, param_name)
                    )
                )
...
def _transform_slice_to_py39(
    slice: ast.expr | ast.Slice,
) -> ast.Slice | ast.expr:
    ...
    def visit_Assign(self, node: ast.Assign) -> None:
        for target in node.targets:
            if (
                ...
                and _to_name_str(_transform_slice_to_py39(target.slice))
                != self.key
            ):
...
class FunctionDefDefaultsVisitor(ast.NodeVisitor):
    ...
    def visit_mutable_literal_or_comprehension(self, node) -> None:
        ...
        if self.arg_depth == 1:
            self.errors.append(
                self.error_code_calls(node.lineno, node.col_offset)
            )
    ...
    def visit_Call(self, node) -> None:
        ...
        if (
            call_path
            in B008_IMMUTABLE_CALLS | self.b008_b039_extend_immutable_calls
        ):
            self.generic_visit(node)
            return
