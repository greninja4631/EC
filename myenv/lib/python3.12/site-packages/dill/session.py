...
def _module_map():
    ...
    for modname, module in sys.modules.items():
        if modname in ('__main__', '__mp_main__') or not isinstance(
            module, ModuleType
        ):
            continue
        if '.' not in modname:
...
def dump_session(filename=None, main=None, byref=False, **kwds):
    warnings.warn(
        "dump_session() has been renamed dump_module()",
    ...
    def peek(self, n):
        ...
        try:
            ...
            raise NotImplementedError(
                "stream is not peekable: %r", stream
            ) from None
        ...
        for opcode, arg, pos in genops(file.peek(256)):
            if not found_import:
                if opcode.name in (
                    'GLOBAL',
                    'SHORT_BINUNICODE',
                ) and arg.endswith('_import_module'):
                    found_import = True
            else:
...
def load_session(filename=None, main=None, **kwds):
    warnings.warn(
        "load_session() has been renamed load_module().",
        PendingDeprecationWarning,
    )
