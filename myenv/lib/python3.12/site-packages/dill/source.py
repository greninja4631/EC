...
def findsource(object):
    ...
    if IS_IPYTHON and is_module_main:
        ...
        try:
            ...
            if isclass(object):
                for object_method in filter(
                    isfunction, object.__dict__.values()
                ):
...
def dumpsource(object, alias='', new=False, enclose=True):
    ...
    if enclose:
        stub = (
            '__this_is_a_stub_variable__'  # XXX: *must* be same _enclose.stub
        )
        pre = '%s = ' % stub
...
def getname(
    obj, force=False, fqn=False
):  # XXX: throw(?) to raise error on fail?
...
def _namespace(obj):
    ...
    try:  # XXX: needs some work and testing on different types
        module = qual = (
            str(getmodule(obj)).split()[1].strip('>').strip('"').strip("'")
        )
        qual = qual.split('.')
        if ismodule(obj):
            return qual
        ...
        if not source:  # we want an import
            try:
                if _isinstance(obj):  # for instances, punt to _importable
                    return _importable(
                        obj, alias, source=False, builtin=builtin
                    )
