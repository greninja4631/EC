...
if NumpyArrayType:  # then has numpy
    ...
    def ndarraysubclassinstance(obj_type):
        if all(
            (c.__module__, c.__name__) != ('numpy', 'ndarray')
    ...
    def numpyufunc(obj_type):
        return any(
            (c.__module__, c.__name__) == ('numpy', 'ufunc')
            for c in obj_type.__mro__
        )

    def numpydtype(obj_type):
        if all(
            (c.__module__, c.__name__) != ('numpy', 'dtype')
            for c in obj_type.__mro__
        ):
            return False
...
class Sentinel(object):
    ...
    def __init__(self, name, module_name=None):
        ...
        if module_name is None:
            ...
            self.__module__ = inspect.currentframe().f_back.f_globals[
                '__name__'
            ]
        else:
...
class Pickler(StockPickler):
    ...
    def save(self, obj, save_persistent_id=True):
        # numpy hack
        obj_type = type(obj)
        if NumpyArrayType and not (
            obj_type is type or obj_type in Pickler.dispatch
        ):
...
class Unpickler(StockUnpickler):
    ...
    def load(self):  # NOTE: if settings change, need to update attributes
        ...
        if type(obj).__module__ == getattr(
            _main_module, '__name__', '__main__'
        ):
            if not self._ignore:
...
def _create_typemap():
    ...
    d = dict(
        list(__builtin__.__dict__.items()) + list(types.__dict__.items())
    ).items()
    for key, value in d:
        if (
            getattr(value, '__module__', None) == 'builtins'
...
def save_module_dict(pickler, obj):
    if (
        ...
        and obj
        is getattr(_import_module(obj['__name__'], True), '__dict__', None)
    ):
...
if ThreadHandleType:
    ...
    def save_thread_handle(pickler, obj):
        ...
        pickler.save_reduce(
            _create_thread_handle, (obj.ident, obj.is_done()), obj=obj
        )
...
if InputType:
    ...
    def save_stringi(pickler, obj):
        ...
        if obj.closed:
            ...
            position = obj.tell()
        pickler.save_reduce(
            _create_stringi, (value, position, obj.closed), obj=obj
        )
    ...
    def save_stringo(pickler, obj):
        ...
        if obj.closed:
            ...
            position = obj.tell()
        pickler.save_reduce(
            _create_stringo, (value, position, obj.closed), obj=obj
        )
        logger.trace(pickler, "# Io")
        return
    ...
    if False:  # _use_diff:
        ...
        if (
            ...
            for item in IPYTHON_SINGLETONS:  # pragma: no cover
                if getattr(main_dict.get(item), '__module__', '').startswith(
                    'IPython'
                ):
                    del main_dict[item]
            pickler.save_reduce(
                _import_module, (mod_name,), obj=obj, state=main_dict
            )
            logger.trace(pickler, "# M1")
...
def save_property(pickler, obj):
    ...
    pickler.save_reduce(
        type(obj), (obj.fget, obj.fset, obj.fdel, obj.__doc__), obj=obj
    )
...
def save_function(pickler, obj):
    if not _locate_function(obj, pickler):
        ...
        _original_main = getattr(
            pickler, '_original_main', __builtin__
        )  #'None'
        postproc_list = []
        if _recurse:
            ...
            # If the globals is a module __dict__, do not save it in the pickle.
                ...
                and getattr(
                    _import_module(obj.__module__, True), '__dict__', None
                )
                is globs_copy
            ):
                globs = globs_copy
...
def check(obj, *args, **kwds):
    ...
    *python* is the path to the python interpreter (defaults to sys.executable)
    ...
    msg = (
        "SUCCESS" if not subprocess.call(msg.split(None, 2)) else "LOAD FAILED"
    )
    if verbose:
        print(msg)
    return
