...
session_file = os.path.join(
    os.path.dirname(__file__), 'session-refimported-%s.pkl'
)
...
def test_session_main(refimported):
    ...
    with TestNamespace(**extra_objects) as ns:
        try:
            ...
            dill.dump_module(
                session_file % refimported, refimported=refimported
            )
...
def test_session_other():
    ...
    dict_objects = [
        obj for obj in module.__dict__.keys() if not obj.startswith('__')
    ]
