...
def test_classes():
    ...
    x = (
        y
        if (IS_PYPY or sys.hexversion >= PY310b)
...
def test_importable():
    ...
    assert (
        importable(squared, source=False)
        == 'from %s import squared\n' % __name__
    )
    assert importable(Foo, source=False) == 'from %s import Foo\n' % __name__
    assert (
        importable(Foo.bar, source=False) == 'from %s import bar\n' % __name__
    )
    assert (
        importable(_foo.bar, source=False) == 'from %s import bar\n' % __name__
    )
    ...
    assert (
        importable(Foo.bar, source=True)
        == 'def bar(self, x):\n  return x*x+x\n'
    )
    assert (
        importable(Foo.bar, source=False) == 'from %s import bar\n' % __name__
    ...
    assert (
        importable(add, builtin=True, source=False)
        == 'from %s import add\n' % __name__
    )
    assert (
    ...
    assert (
        importable(Foo, builtin=True, source=False)
        == 'from %s import Foo\n' % __name__
    )
    assert (
...
def test_numpy():
    try:
        ...
        x = y([1, 2, 3])
        assert (
            importable(x, source=False)
            == 'from numpy import array\narray([1, 2, 3])\n'
        )
        assert (
            importable(y, source=False)
            == 'from %s import array\n' % y.__module__
        )
        assert (
            importable(x, source=True)
            == 'from numpy import array\narray([1, 2, 3])\n'
        )
        assert (
            importable(y, source=True)
            == 'from %s import array\n' % y.__module__
        )
        y = np.int64
        x = y(0)
        assert (
            importable(x, source=False) == 'from numpy import int64\nint64(0)\n'
        )
        assert (
            importable(y, source=False)
            == 'from %s import int64\n' % y.__module__
        ...
        assert (
            importable(x, source=True) == 'from numpy import int64\nint64(0)\n'
        )
        assert (
            importable(y, source=True)
            == 'from %s import int64\n' % y.__module__
