...
def test_getsourcelines():
    ...
    assert (
        ''.join(ds.getsourcelines(ts.add)[0]) == 'def add(x,y):\n  return x+y\n'
    )
    assert ''.join(ds.getsourcelines(squared)[0]) == 'squared = lambda x:x**2\n'
    assert (
        ''.join(ds.getsourcelines(ts.squared)[0]) == 'squared = lambda x:x**2\n'
    )
    assert ''.join(ds.getsourcelines(Bar)[0]) == 'class Bar:\n  pass\n'
    assert ''.join(ds.getsourcelines(ts.Bar)[0]) == 'class Bar:\n  pass\n'
    assert (
        ''.join(ds.getsourcelines(_bar)[0]) == 'class Bar:\n  pass\n'
    )  # XXX: ?
    assert (
        ''.join(ds.getsourcelines(ts._bar)[0]) == 'class Bar:\n  pass\n'
    )  # XXX: ?
    assert (
