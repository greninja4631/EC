...
dont_memo = set(
    id(i)
    for i in (memo, sys.modules, sys.path_importer_cache, os.environ, id_to_obj)
)
...
def get_attrs(obj):
    ...
    """
    if (
        type(obj) in builtins_types
        or type(obj) is type
        and obj in builtins_types
    ):
        return
...
def release_gone():
    ...
    [
        (itop(id_), mp(id_))
        for id_, obj in list(id_to_obj.items())
        if src(obj) < 4
